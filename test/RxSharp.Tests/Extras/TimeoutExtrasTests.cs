...
public class TimeoutExtrasTests
{
    ...
    public void ShouldErrorAfterTheDelay()
    {
        ...
        PuppeteerExtras.Timeout(TimeSpan.FromMilliseconds(20)).Subscribe(onError: err =>
        {
            received = err;
            signal.Set();
        });
    ...
    {
        var errored = false;
        PuppeteerExtras.Timeout(TimeSpan.Zero).Subscribe(onError: _ => errored = true);
    ...
    public void ShouldUseTheCustomCauseFactory()
    {
        ...
        PuppeteerExtras.Timeout(TimeSpan.FromMilliseconds(10), () => cause).Subscribe(onError: err =>
        {
            received = err;
            signal.Set();
        });
