...
public static class CombineLatestAllOperator
{
    ...
    /// completes, combines all of them with <see cref="RxSharp.Observable.CombineLatest{T}"/>: every time any
    ...
    /// </summary>
    /// <remarks>
    /// Same-type-only, like <see cref="RxSharp.Observable.CombineLatest{T}"/> itself: this does not port rxjs's
    ...
    public static Observable<IReadOnlyList<T>> CombineLatestAll<T>(this Observable<Observable<T>> source)
        => source.ToArray().MergeMap(sources => RxSharp.Observable.CombineLatest(sources.ToArray()));
}
