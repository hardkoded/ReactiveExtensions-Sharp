...
public static class CombineLatestWithOperator
{
    ...
    /// of them emits thereafter. Pipeable-operator sugar over <see cref="RxSharp.Observable.CombineLatest{T}"/>
    ...
    /// </summary>
    /// <remarks>
    /// Same-type-only, like <see cref="RxSharp.Observable.CombineLatest{T}"/> itself, rather than rxjs's
    ...
    public static Observable<IReadOnlyList<T>> CombineLatestWith<T>(this Observable<T> source, params Observable<T>[] otherSources)
        => RxSharp.Observable.CombineLatest(Prepend(source, otherSources));
