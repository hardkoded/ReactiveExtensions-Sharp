...
public static class ZipWithOperator
{
    ...
    /// <see cref="RxSharp.Observable.Zip{T}"/> with <paramref name="source"/> prepended:
    /// <c>source.ZipWith(a, b)</c> is the same as <c>Observable.Zip(source, a, b)</c>.
    /// </summary>
    /// <remarks>
    /// Same-type-only, like <see cref="RxSharp.Observable.Zip{T}"/> itself, rather than rxjs's
    ...
    public static Observable<IReadOnlyList<T>> ZipWith<T>(this Observable<T> source, params Observable<T>[] otherSources)
        => RxSharp.Observable.Zip(Prepend(source, otherSources));
