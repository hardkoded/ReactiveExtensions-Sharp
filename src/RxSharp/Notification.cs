...
/// </summary>
/// <remarks>
/// This lives in the main <c>RxSharp</c> namespace, not <c>RxSharp.Testing</c>, since materialize/dematerialize
/// are core operators rather than test-only utilities. <c>RxSharp.Testing</c> has a superficially similar type,
/// <see cref="RxSharp.Testing.Recorded{T}"/>, but that type also carries a <see cref="RxSharp.Testing.VirtualTimeScheduler"/>
/// clock timestamp that has no meaning here, and core types should not depend on the testing namespace — so
/// this is a separate, deliberately simpler type rather than a reuse of <see cref="RxSharp.Testing.Recorded{T}"/>.
/// </remarks>
