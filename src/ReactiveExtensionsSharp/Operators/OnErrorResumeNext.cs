...
public static class OnErrorResumeNextOperator
{
    ...
    /// <see cref="ReactiveExtensionsSharp.Observable.OnErrorResumeNext{T}"/> with <paramref name="source"/> prepended:
    ...
    public static Observable<T> OnErrorResumeNext<T>(this Observable<T> source, params Observable<T>[] otherSources)
        => ReactiveExtensionsSharp.Observable.OnErrorResumeNext(Prepend(source, otherSources));
