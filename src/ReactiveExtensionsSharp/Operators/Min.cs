...
/// <summary>Extension methods implementing the <c>min</c> operator.</summary>
/// <remarks>
/// Built directly on the no-seed <see cref="ReduceOperator.Reduce{T}(Observable{T}, Func{T, T, T})"/> overload,
/// matching rxjs's own <c>min.ts</c> (itself a thin wrapper over <c>reduce</c>). Since that overload emits nothing
/// for an empty source rather than erroring, so does <see cref="Min{T}(Observable{T})"/> — matches rxjs exactly, verified
/// against the 7.8.2 tag rather than assumed.
/// </remarks>
public static class MinOperator
{
    /// <summary>
    /// Emits the single smallest value seen from <paramref name="source"/>, once it completes, using the
    /// default <see cref="Comparer{T}"/> for <typeparamref name="T"/>.
    /// </summary>
    ...
    /// <returns>An observable that emits the smallest value seen, once <paramref name="source"/> completes.</returns>
    public static Observable<T> Min<T>(this Observable<T> source)
        => source.Min(Comparer<T>.Default.Compare);

    /// <summary>
    /// Emits the single smallest value seen from <paramref name="source"/> (per <paramref name="comparer"/>),
    /// once it completes.
    /// </summary>
    /// <remarks>
    ...
    /// <typeparam name="T">The type of values emitted by <paramref name="source"/> and of the output.</typeparam>
    /// <param name="source">The source sequence.</param>
    /// <param name="comparer">A function returning a negative number if the first value is smaller, matching <see cref="Comparer{T}.Compare(T, T)"/>'s contract.</param>
    /// <returns>An observable that emits the smallest value seen, once <paramref name="source"/> completes.</returns>
