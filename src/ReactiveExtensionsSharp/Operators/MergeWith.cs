...
public static class MergeWithOperator
{
    ...
    /// <see cref="ReactiveExtensionsSharp.Observable.Merge{T}"/> with <paramref name="source"/> prepended:
    ...
    public static Observable<T> MergeWith<T>(this Observable<T> source, params Observable<T>[] otherSources)
        => ReactiveExtensionsSharp.Observable.Merge(Prepend(source, otherSources));
