...
public static class ZipAllOperator
{
    ...
    /// completes, combines all of them with <see cref="ReactiveExtensionsSharp.Observable.Zip{T}"/>: the Nth value from every
    ...
    /// </summary>
    /// <remarks>
    /// Same-type-only, like <see cref="ReactiveExtensionsSharp.Observable.Zip{T}"/> itself — see <see cref="CombineLatestAllOperator.CombineLatestAll{T}"/>'s
    ...
    public static Observable<IReadOnlyList<T>> ZipAll<T>(this Observable<Observable<T>> source)
        => source.ToArray().MergeMap(sources => ReactiveExtensionsSharp.Observable.Zip(sources.ToArray()));
}
