...
public static class ConcatWithOperator
{
    ...
    /// <see cref="ReactiveExtensionsSharp.Observable.Concat{T}"/> with <paramref name="source"/> prepended:
    ...
    public static Observable<T> ConcatWith<T>(this Observable<T> source, params Observable<T>[] otherSources)
        => ReactiveExtensionsSharp.Observable.Concat(Prepend(source, otherSources));
